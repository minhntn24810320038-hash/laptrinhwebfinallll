document.addEventListener("DOMContentLoaded", function () {
  ...
  // ==========================================
  // 2. KHỞI TẠO BIẾN & DANH SÁCH SIZE PHỤ KIỆN
  // ==========================================
  let cart = JSON.parse(localStorage.getItem("gentleman_cart")) || [];

  // Thước đo linh động cho phụ kiện nam (Freesize hoặc Chu vi nhẫn/vòng)
  const accessorySizes = ["Freesize", "56mm", "58mm", "60mm"];
    ...
    cartItemsContainer.querySelectorAll(".minus-qty").forEach((btn) => {
      btn.addEventListener("click", function (e) {
        ...
        if (cart[index].quantity > 1) {
          ...
          if (confirm("Bro muốn bỏ phụ kiện này ra khỏi giỏ chứ?")) {
            cart.splice(index, 1);
          }
        }
        saveAndRefreshCart();
  ...
  // ==========================================
  // 5. MỞ POPUP CHI TIẾT KHI CLICK SẢN PHẨM
  // ==========================================
  const productCards = document.querySelectorAll(".product-card");
  productCards.forEach((card) => {
    card.addEventListener("click", function () {
      currentProduct = {
        id: this.getAttribute("data-id") || "TS0",
        name:
        ...
        code: this.getAttribute("data-code") || "TS-00",
      };
      ...
      if (sizeContainer) {
        sizeContainer.innerHTML = accessorySizes
          .map(
            (size) =>
  ...
  // ==========================================
  // 6. THÊM VÀO GIỎ HÀNG CHUNG (LOCALSTORAGE)
  // ==========================================
  function executeAddingProduct() {
    if (!selectedSize) {
      alert("Quý ông vui lòng bấm chọn một thông số kích cỡ trước!");
      return false;
    }
